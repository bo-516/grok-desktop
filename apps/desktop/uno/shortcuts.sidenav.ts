...
export const sideNavShortcuts: Record<string, string> = {
  ...
  "side-nav-scroll":
    "flex-1 overflow-y-auto min-h-0 pl-2.5 pr-3.5 pb-2 flex flex-col gap-0.5",
  "side-nav-empty": "px-2.5 py-4 text-12px text-fg-muted leading-snug",
  /*
   * Project tree (session rail): sticky section head → folder header → nested
   * chats with a left guide. Header is a quiet row (not a selection pill);
   * selected session uses elevated fill + left accent so hierarchy is clear.
   * The head is opaque (bg-sidebar) because rows scroll underneath it.
   */
  "project-section-head":
    "sticky top-0 z-10 flex items-center justify-between gap-2 pt-2 pb-1.5 bg-sidebar",
  ...
  /* gap-1.5 keeps the count badge clear of the truncated name; pr-1.5
   * mirrors the row's right inset so 3-digit counts don't hug the rail. */
  "project-group-header":
    "relative flex items-center gap-1.5 min-h-8 pr-1.5 rounded-8px transition-colors duration-fast ease-soft hover:bg-white-faint",
  "project-group-main":
  ...
  "project-group-name":
    "min-w-0 flex-1 pr-0.5 text-nav font-medium text-fg whitespace-nowrap overflow-hidden text-ellipsis leading-snug",
  ...
  /*
   * Session rows: status | title | pin | time-or-remove.
   * Pin is its own column so it never stacks with remove. Height uses px
   * (html font-size is 13px; rem h-* collapses). Active = elevated fill +
   * left accent bar (not the same chrome as folder).
   */
  "sess-row":
    "relative grid grid-cols-[auto_minmax(0,1fr)_auto_auto] items-center gap-1 w-full h-[32px] pl-2 pr-1.5 rounded-7px cursor-pointer text-fg-secondary transition-colors duration-normal ease-soft hover:(text-fg bg-white-faint) data-[dragging=true]:(opacity-45) data-[drag-over=true]:(bg-white-soft shadow-[inset_0_2px_0_0_var(--color-text-primary)])",
  "sess-row-active":
    "bg-high text-fg shadow-[inset_2px_0_0_0_var(--color-text-primary)] hover:(bg-high text-fg)",
  ...
  /* Dot is a running/waiting cue, not a bullet: idle only hints on hover. */
  "sess-status":
    "shrink-0 w-1.5 h-1.5 rounded-full bg-fg-faint transition-all duration-fast ease-soft",
  "sess-status-idle": "opacity-0 group-hover:opacity-40",
  "sess-status-live":
    "bg-fg opacity-100 shadow-[0_0_0_3px_var(--color-accent-muted)] animate-pulse",
  "sess-status-wait": "bg-warning opacity-100",
  "sess-title":
   ...
   * Time sits a hair left of the slot edge (`pr-1`) so "1h" / "5h" don't hug
   * the rail padding; the remove icon stays flush right on purpose.
   */
  "sess-meta":
    "relative shrink-0 flex items-center justify-end w-[26px] h-[20px]",
  "sess-time":
    "shrink-0 pr-1 text-10px leading-none text-fg-faint whitespace-nowrap tabular-nums text-right transition-opacity duration-fast ease-soft group-hover:opacity-0 group-focus-within:opacity-0",
  /*
   * Remove stays flush to the meta slot's right edge (unlike `sess-time`, which
   * is inset). `p-0` is required: native button padding would shift the icon.
   */
  "sess-remove":
    "absolute inset-0 flex items-center justify-end p-0 m-0 border-none bg-transparent text-fg-muted rounded-6px opacity-0 pointer-events-none transition-opacity duration-fast ease-soft group-hover:(opacity-100 pointer-events-auto) group-focus-within:(opacity-100 pointer-events-auto) hover:(text-fg bg-white-soft) focus-visible:(opacity-100 pointer-events-auto outline-none ring-2 ring-[var(--color-focus-ring)])",
  "sess-remove-icon": "w-3 h-3 block shrink-0",
  "side-nav-footer":
