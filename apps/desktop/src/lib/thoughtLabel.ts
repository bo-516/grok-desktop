...
/**
 * Live label of a streaming thought row (Codex-style single word).
 *
 * Deliberately bare: the row renders its body right underneath while live, and
 * the turn header plus the composer strip already carry a current-step preview,
 * so repeating the first reasoning line here was the third copy of it. The
 * shimmer in ThoughtWidget — not extra words — is what marks it as live.
 */
export const THOUGHT_LIVE_LABEL = "Thinking";

/**
 * Builds the Thought toggle label from lifecycle fields.
 * Live rows are the bare {@link THOUGHT_LIVE_LABEL}; completed rows are
 * duration-only (or bare `Thought` when timestamps are missing) so history
 * stays quiet.
 ...
 * @returns `Thinking` / `Thought for Ns` / `Thought`.
 */
