// Package bridgeauth implements pure token / Origin / listen-port helpers for
// the local bridge WebSocket upgrade path (no HTTP server wiring).
package bridgeauth
...
// Config is injectable auth configuration for the bridge WebSocket upgrade.
...
// Result is the outcome of validating an inbound WS upgrade.
// OK true means the connection may proceed; Status is the HTTP status when rejected.
type Result struct {
	OK     bool
	Reason string
	Status int
...
// envToken is trimmed; empty / whitespace yields a fresh 24-byte base64url token.
...
// First non-empty source wins (query → RequestURI query → header → Bearer).
...
// AuthorizeConnection validates token + Origin for an inbound WebSocket upgrade.
// Returns OK=false with Status 403 (origin) or 401 (token) on failure.
func AuthorizeConnection(r *http.Request, config Config) Result {
	...
	if !IsOriginAllowed(origin, config.AllowedOrigins) {
		...
		return Result{OK: false, Reason: "origin not allowed: " + o, Status: 403}
	}
	token := ExtractTokenFromRequest(r)
	if token == "" {
		return Result{OK: false, Reason: "missing bridge token", Status: 401}
	}
	if token != config.Token {
		return Result{OK: false, Reason: "invalid bridge token", Status: 401}
	}
	return Result{OK: true}
}

// BridgeWsURL builds the public WS URL clients should use (token in query).
// host defaults to 127.0.0.1 when empty.
